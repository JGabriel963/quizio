...
- [x] **T01** `[P]` `core/shared` — texto pesquisável e contagem de caracteres
...
- [x] **T02** `[P]` `core/shared` — chave de mídia por dono
...
- [x] **T03** `[P]` `core/identity` — regras de cadastro
...
- [x] **T04** `core/quiz` — dados do quiz (value objects) — depende de T01
...
- [x] **T05** `core/quiz` — agregado `Quiz` — depende de T04
...
- [x] **T06** `[P]` `core/library` — seções e pesquisa — depende de T01
...
- [x] **T07** `core` — portas e fakes — depende de T05
...
- [x] **T08** `core/quiz` — criar quiz — depende de T02, T07
...
- [x] **T09** `[P]` `core/quiz` — detalhes do quiz — depende de T07
...
- [x] **T10** `core/quiz` — editar dados e capa — depende de T02, T07
...
- [x] **T11** `[P]` `core/quiz` — duplicar — depende de T02, T07
...
- [x] **T12** `[P]` `core/quiz` — lixeira e restaurar — depende de T07
...
- [x] **T13** `[P]` `core/quiz` — excluir definitivamente — depende de T07
...
- [x] **T14** `[P]` `core/library` — listar biblioteca — depende de T06, T07
...
- [x] **T15** `db` — schema `quiz` e repositório — depende de T05, T07
...
- [x] **T16** `db` — consulta da biblioteca — depende de T14, T15
...
- [x] **T17** `[P]` `storage` — copiar objeto — depende de T07
...
- [x] **T18** `auth` — fábrica `createAuth`, cadastro e login por e-mail — depende de T03
...
- [x] **T19** `auth` — limite de tentativas persistido — depende de T18
...
- [x] **T20** `auth` — Google, vínculo e cadastro desligado — depende de T18
...
- [x] **T21** `api` — router `quiz`, erros `NOT_FOUND` e ligações — depende de T08–T13, T15
  ...
  - Implementar: `NotFoundError → NOT_FOUND` no middleware de `src/index.ts`; `src/routers/quiz.ts`; `Adapters`/`useCases` em `container.ts`; repositório Drizzle e `db` compartilhado em `composition-root.ts` (a remoção de `privateData` foi movida para T26, junto com a rota de dashboard que ainda o usa)
  - Cobre: CA-14, CA-19, CA-24, CA-26, RN-10

- [x] **T22** `[P]` `api` — router `library` — depende de T14, T16, T21
...
- [x] **T23** `[P]` `api` — router `auth.settings` — depende de T20, T21
...
- [x] **T24** `[P]` `ui` — componentes novos no design system
...
- [x] **T26** `web` — telas de entrada — depende de T20, T23, T24, T25
...
- [x] **T27** `web` — página da biblioteca — depende de T22, T24, T25
...
- [x] **T28** `web` — criar e editar dados do quiz com capa — depende de T21, T24, T25, T27
...
- [x] **T29** `web` — detalhes do quiz, lixeira e exclusão definitiva — depende de T27, T28
...
- [x] **T30** `[P]` `web/e2e` — fluxos de conta — depende de T26, T27
...
- [x] **T31** `[P]` `web/e2e` — fluxos da biblioteca — depende de T28, T29
...
- [x] **T32** `[P]` Google OAuth — configuração e verificação manual — depende de T26
  - **Status (2026-09-14):** instruções entregues em `apps/web/.env.example` e no checklist de deploy de `specs/roadmap.md`. O **smoke manual com o Google real ainda não foi feito**, porque não há cliente OAuth neste ambiente; ele foi movido para o checklist de deploy. Os CA-07/08/09 têm cobertura automatizada via login por ID token (T20).
